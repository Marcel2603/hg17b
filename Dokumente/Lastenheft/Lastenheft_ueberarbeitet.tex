...
\paragraph{Back-End} Wir erstellen eine Datenbank im RDF-Format zum Speichern der Teilnehmer in Form von anonymen IDs, deren besuchte Veranstaltungen und eventuell ihrer Punktestände, auf die die App zugreifen kann. Für die Anbindung wird auf dem Server TDB und Fuseki aus dem Apache Jena Framework verwendet. TDB dient dabei als (lokaler) Triple Store, der mittels Fuseki über HTTP als eingebetteter SPARQL Endpunkt erreichbar ist. Zunächst wird diese Datenbank auf dem Praktikumsserver implementiert. Später soll jedoch die Möglichkeit bestehen, die Datenbank auf einen anderen Server zu verschieben.
\paragraph{Dokumentation} Das Projekt wird dokumentiert, sowohl um den Endnutzern die Bedienung zu erleichtern, als auch um die Weiterentwicklung durch andere Entwickler zu vereinfachen.
...
\sortitem[Backend] {Backende beschreibt das auf dem Server laufende Programm, ein RDF Triple Store (TDB, Apache Jena) sowie ein über HTTP erreichbarer eingebetteter SPARQL Endpunkt (Fuseki, Apache Jena)}
...
\sortitem[Veranstalter] {Der Veranstalter organisiert Freizeitaktivitäten für die Schüler. Er soll mit der App den Code des Schülers einscannen und seine Veranstaltungen registrieren können.}
\end{sortedlist}
